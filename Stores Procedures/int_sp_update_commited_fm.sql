create or replace procedure INT_SP_UPDATE_COMMITED_FM
IS
...
CURSOR c_amount_pc IS
       ...
       where TYPE_AMOUNT='MC' and EFFECTIVE_DATE is not null
...
execute immediate 'alter session set NLS_NUMERIC_CHARACTERS= ''.,'' ';
 ...
 OPEN c_amount_pc;
   ...
   EXIT WHEN c_amount_pc%NOTFOUND;
        ...
        AND to_char(EFFECTIVE_DATE,'MM')=to_char(r_amount_pc.EFFECTIVE_DATE,'MM')
                            AND to_char(EFFECTIVE_DATE,'YYYY')=to_char(r_amount_pc.EFFECTIVE_DATE,'YYYY');
   END LOOP;
   CLOSE c_amount_pc;
END;
